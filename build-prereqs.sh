...
echo "=== 3. Downloading OS Packages (Offline Installers & Dependencies) ==="
if [[ "$OS" =~ ^(ubuntu|debian)$ ]]; then
    sudo apt-get update -y -qq
    sudo apt-get install -y -qq software-properties-common apt-transport-https ca-certificates curl
    ...
    cd "${BUNDLE_DIR}/packages"
    echo "Resolving and downloading all RHEL/CentOS dependencies..."
    sudo yumdownloader --resolve --alldeps --quiet --destdir=. docker-ce docker-ce-cli containerd.io docker-compose-plugin socat conntrack skopeo
    cd ../..
fi
